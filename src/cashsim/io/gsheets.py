...
def _choose_creds(*, readonly: bool, sa_info: Mapping[str, Any] | None):
...
def batch_read_values(
    ...
    """
    sh = gs_client(sa_info, readonly=True).open_by_url(sheet_url)
...
def batch_write_values(
    ...
    *,
    ...
    """
    sh = gs_client(sa_info, readonly=False).open_by_url(sheet_url)
...
def read_wishlist_by_url(
    ...
    df: pd.DataFrame = get_as_dataframe(ws, evaluate_formulas=True, header=0).dropna(how="all")
    ...
    df["Days I have to Buy Again"] = pd.to_numeric(df["Days I have to Buy Again"], errors="coerce").fillna(0).astype(int)
    return df[wanted]
...
def write_dataframe(
    ...
    *,
    ...
    client = gs_client(sa_info, readonly=False)
    ws = client.open_by_url(sheet_url).worksheet(worksheet_name)
    if clear_first:
        ws.clear()
